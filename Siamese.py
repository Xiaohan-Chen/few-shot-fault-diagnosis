...
Attention: it is not a one-shot learning method, but a one-shot recognization method.
"""
import argparse
import logging
import warnings
...
def parse_args():
    ...
    # training set
    parser.add_argument("--support_s", type=int, default=200, help="the number of training samples per class")
    # test set
    parser.add_argument("--support_t", type=int, default=100, help="the number of test samples per class")
...
def loaddata(args):
    if args.source_dataname == "CWRU":
        source_data = CWRUloader(args, args.s_load, args.s_label_set, args.support_s)
    
    if args.target_dataname == "CWRU":
        target_data = CWRUloader(args, args.t_load, args.t_label_set, args.support_t)
